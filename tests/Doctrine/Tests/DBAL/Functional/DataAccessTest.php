...
class DataAccessTest extends \Doctrine\Tests\DbalFunctionalTestCase
{
    ...
    public function testFetchAllWithMissingTypes()
    {
        ...
        $sql = "SELECT test_int, test_datetime, test_date, test_time FROM fetch_table WHERE test_int = ? AND test_datetime = ? AND test_date = ? AND test_time = ?";
        $data = $this->_conn->fetchAll($sql, array(1, $datetime, $date, $time));
    }
    ...
    public function testFetchAssocWithMissingTypes()
    {
        ...
        $row = $this->_conn->fetchAssoc($sql, array(1, $datetime, $date, $time));
    }
    ...
    public function testFetchArrayWithMissingTypes()
    {
        ...
        $sql = "SELECT test_int, test_datetime, test_date, test_time FROM fetch_table WHERE test_int = ? AND test_datetime = ? AND test_date = ? AND test_time = ?";
        $row = $this->_conn->fetchArray($sql, array(1, $datetime, $date, $time));
    }
...
class MyFetchClass
{
    public $test_int, $test_string, $test_datetime, $test_date;
    private $test_time;
